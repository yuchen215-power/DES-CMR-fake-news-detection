...
from Models.FCWithEvidences import getral_prompt_dy_topk15_entropy as getral
...
def fit_models(args):
    ...
    curr_date = datetime.now().timestamp()  # seconds
    # folder to store all outputed files of a run
    # secondary_log_folder = os.path.join(args.log, "log_results_%s" % (args.dataset))
    secondary_log_folder = os.path.join(args.log, f"log_results_prompt_dy_topk15_entropy_{args.dataset}_{curr_date}")
    ...
    dataset_root = os.path.join(args.path, args.dataset)
    candidate_roots = [
        os.path.join(dataset_root, "mapped_data"),
    ]
    root = next((path for path in candidate_roots if os.path.isdir(path)), None)
    if root is None:
        raise FileNotFoundError(
            "No prepared data directory found for %s under %s" % (args.dataset, dataset_root)
        )
    tx = time.time()
    ...
    for i in range(args.num_folds):
        ...
        file_handler.myprint(datetime.now().strftime('%Y-%m-%d %H:%M: ') + "======================== FOLD %s ========================" % i)
        ...
        train_interactions = ClassificationInteractions(train_processed, **additional_data)         # transform to list(interactions)
        ...
        match_params["n_kernel"] = args.kernel_number
        match_params["alpha"] = args.alpha
        match_params["tau"] = args.tau

        # ------------------------
        match_params["embedding_freeze"] = True
        match_params["output_size"] = 2
        match_params["topk"] = args.topk
        match_params["prompt_dim"] = args.prompt_dim
        match_model = getral.GETRAL(match_params)
    ...
    file_handler.myprint(datetime.now().strftime('%Y-%m-%d %H:%M: ') + "Average results from %s folds" % args.num_folds)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log', default = "logs/getral", type = str, help = 'folder for logs and saved models')
    ...
    # GNN related
    parser.add_argument('--gnn_window_size', type=int, default=3, help="GNN window size")
    ...
    parser.add_argument("--kernel_number", type=int,default=11, help="Number of Gaussian Kernels" )
    parser.add_argument("--alpha", type=float, default=0.5,help="Coefficient to control the score")
    
    # contrastive learning relatedx
    parser.add_argument("--tau", type=float, default=0.4, help="The temperature parameter of contrastive loss")
    parser.add_argument("--con_lambda", type=float, default=0.10)
    parser.add_argument("--mask_rate", type=float, default=0.0, help="Evidence token masking rate for consistency learning")
    parser.add_argument("--consistency_lambda", type=float, default=0.0, help="Weight of masked evidence consistency loss")
    
    parser.add_argument("--topk", type=int, default=15, help="Top K for dynamic evidence selection")
    parser.add_argument("--prompt_dim", type=int, default=128, help="Prompt dimension for cross attention")
