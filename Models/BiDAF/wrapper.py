...
class GGNN2_with_GSL(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, rate=0.8, dropout=0.2, n_kernel=21, alpha=0.5):
        super(GGNN2_with_GSL, self).__init__()

        self.feat_prop1 = GGNN(input_dim, hidden_dim, dropout)
        self.gaussain_kernel = Gaussian_Kernel_Pooling(n_kernel=n_kernel, use_exact=True)
        self.feat_prop2 = h_GGNN(hidden_dim, output_dim, n_kernel, rate, dropout, alpha=alpha)
    
    def forward(self, adj, feat, cfeat, mask, cmask):
        """
        adj: (n1+...+nk, 100, 100)
        feat: (n1+...+nk, 100, input_dim)
        cfeat: (n1+...+nk, 30, input_dim)
        mask: (n1+...+nk, 100)
        cmask: (n1+...+nk, 30,)
        """
        feat = self.feat_prop1(adj, feat)
        affinity_feat = self.gaussain_kernel(feat, cfeat, cmask, cmask)       
        feat = self.feat_prop2(adj, feat, affinity_feat)
        return feat
...
class h_GGNN(GGNN):
    def __init__(self, in_features, out_features, n_kernel, rate=1.0, dropout=0.2,alpha=0.5):
        super(h_GGNN, self).__init__(in_features, out_features, dropout)
        self.rate = rate
        self.word_scorer1 = GGNN(in_features, 1, dropout)
        self.word_scorer2 = GGNN(n_kernel, 1, dropout)
        self.alpha = torch.tensor(alpha)

    def forward(self, adj, x, affinity_feat):
        if hasattr(self, 'dropout'): 
            x = self.dropout(x)

        score1 = self.word_scorer1(adj, x)
        score2 = self.word_scorer2(adj, affinity_feat)
        score = (1 - self.alpha) * score1 +  self.alpha * score2      # fixed coefficient to fuse
        score = torch.sigmoid(score)

        BATCH_SIZE, N, D = x.shape
        num_preserve_node = int(self.rate * N)
        _, indices = score.topk(num_preserve_node, 1)
        indices = torch.squeeze(indices, dim=-1)               # (B, N*k)
        indices, _ = torch.sort(indices, descending=False)     # Ascending order indices

        x = self.proj(x)
        mask1 = torch.zeros(BATCH_SIZE, N).cuda().scatter(dim=1, index=indices, value=1)    # (B, N)
        x1 = x * mask1.unsqueeze(-1)                                                 # (B, N, D)
        adj1 = adj * mask1.unsqueeze(1) * mask1.unsqueeze(-1)        # Only preserve the topk nodes and neighbours                                                         
        score1 = score * mask1.unsqueeze(-1)                                              # (B, N*k, 1)
        x1  = x1 * score1                                # integrate score
        # feat = torch.tanh(score) * feat
        # print(x.shape, x1.shape, adj1.shape, score1.shape) 
...
class GGNN_2(nn.Module):
    def __init__(self, in_features, concat_features, out_features, dropout=0.2):
        super(GGNN_2, self).__init__()
        self.proj = Linear(in_features + concat_features, out_features, bias=False)
        self.linearz0 = Linear(out_features, out_features)
        self.linearz1 = Linear(in_features, out_features)
        self.linearr0 = Linear(out_features, out_features)
        self.linearr1 = Linear(in_features, out_features)
        self.linearh0 = Linear(out_features, out_features)
        self.linearh1 = Linear(in_features, out_features)
        
        if dropout > 0:
            self.dropout = nn.Dropout(p=dropout)

    def forward(self, adj, x, t):
        if hasattr(self, 'dropout'): 
            x = self.dropout(x)
        (n, l, d) = x.shape
        x1 = torch.cat([x, t.unsqueeze(1).repeat(1,l,1)], dim=-1)
...
class Gaussian_Kernel_Pooling(nn.Module):

    def __init__(self, n_kernel, lamb=0.5, use_exact=True):
        """
        create the mu for each guassian kernel. Mu is the middle of each bin
        :param n_kernels: number of kernels (including exact match). first one is exact match
        :return: l_mu, a list of mu.
        """
        super(Gaussian_Kernel_Pooling, self).__init__()

        # init mu and sigma
        l_mu = [1] if use_exact else [2]
        l_sigma = [0.001]  # for exact match. small variance -> exact match
        if n_kernel > 1:
            bin_size = 2.0 / (n_kernel - 1)  # score range from [-1, 1]
            l_mu.append(1 - bin_size / 2)  # mu: middle of the bin
            for i in range(1, n_kernel - 1):
                l_mu.append(l_mu[i] - bin_size)
        l_sigma += [0.1] * (n_kernel - 1)

        self.l_mu = torch.tensor(l_mu)
        self.l_sigma = torch.tensor(l_sigma)
       
    def forward(self, m1, m2, m1_mask, m2_mask):
        """
        m2_mask: (n1+...+nk, len2,); 0 for padding
        """

        norm_m1 = m1.div(torch.norm(m1, p=2, dim=-1, keepdim=True))
        norm_m2 = m2.div(torch.norm(m2, p=2, dim=-1, keepdim=True))
        simi = torch.matmul(norm_m1, norm_m2.transpose(1,2))
        m2_len = torch.sum(m2_mask, dim=1, keepdim=True).unsqueeze(1)   # (n1+...+nk, 1, 1)
        m2_rmask = (~m2_mask).unsqueeze(1).repeat(1,100,1)
        
        feat = []
        for mu, sigma in zip(self.l_mu, self.l_sigma):
            tmp = torch.exp(- torch.square(simi-mu) / (2*torch.square(sigma)))  # exp(- (x-mu)^2 /(2*sigmma^2) )
            tmp = tmp.masked_fill(m2_rmask, 0)
            feat.append(torch.sum(tmp, dim=-1, keepdim=True))
        
        feat = torch.cat(feat, dim=-1).div(m2_len)                  # (n1+...+nk, len1, len_mu)
        return feat
