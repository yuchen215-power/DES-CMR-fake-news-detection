...
class GETRAL(BasicFCModel):
    ...
    def __init__(self, params):
        ...
        self.prompt_proj = nn.Linear(D + self.prompt_dim,
                                     D)
        # 动态证据选择
        self.evd_selector = DynamicEvidenceSelector(embedding_dim=D, top_k=5,num_heads=self.num_att_heads_for_evds)
    ...
    def forward(self, query: torch.Tensor, document: torch.Tensor, verbose=False, **kargs):
        ...
        selected_evidences = self.evd_selector(query_trans, evidences)
        padded_doc_out = torch.stack([ev.permute(1, 0, 2) for ev in selected_evidences], dim=1)
        B, n, R, D = padded_doc_out.size()
        # Keep embed_q as-is for downstream steps
        kargs["enhanced_query_embed"] = embed_q
